...
class ApiClient {
    ...
    const response = await this.request<ApiSignupResponse>("/auth/register", {
      ...
      body: JSON.stringify({
        ...
        privacy_consent: {
          ...
          optional_agreed: data.optionalPrivacyConsent ?? false,
        },
      }),
    }, false);
    return {
  ...
  createApplication(
    ...
    return this.request<ApiApplicationDetail>("/applications", {
      method: "POST",
      body: JSON.stringify({ form_id: formId, is_draft: isDraft, answers }),
    });
  }
  ...
  patchApplication(
    ...
    return this.request<ApiApplicationDetail>(`/applications/${encodeURIComponent(applicationId)}`, {
      method: "PATCH",
      body: JSON.stringify({ answers, ...(isDraft === undefined ? {} : { is_draft: isDraft }) }),
    });
  }
